...
contract XBettingNFTUC is ERC721, UniversalChanIbcApp {
    ...
    constructor(address _middleware, string memory _baseURI) 
        ERC721("XBettingNFTUC", "XBettingNFTUC")
    ...
    function onRecvUniversalPacket(
       ...
       // Decode the packet data
        (address decodedRecipient) = abi.decode(packet.appData, (address));

        // Mint the NFT
        uint256 nftId = mint(decodedRecipient);
        emit MintedOnRecv(IbcUtils.toAddress(packet.srcPortAddr), decodedRecipient, nftId);

        // Encode the ack data
        bytes memory ackData = abi.encode(decodedRecipient, nftId);
